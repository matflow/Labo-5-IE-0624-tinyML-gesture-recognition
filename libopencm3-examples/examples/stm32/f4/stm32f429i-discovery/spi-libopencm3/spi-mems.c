...
read_xyz(int16_t vecs[3])
{
	...
	vecs[0] = (buf[1] << 8 | buf[0]);  // mascara con HI y LO axis
...
int main(void)
{
	...
	baseline[0] = 0;
	baseline[1] = 0;
	baseline[2] = 0;     // a partir de aca usar main de /f3
	...
	while (1) {
		tmp = read_xyz(vecs);
		for (i = 0; i < 3; i++) {         // aquí se escriben a consola los ejes y valor
			int pad;
			console_puts(axes[i]);  // pone 'X: '
			tmp = vecs[i] - baseline[i];
			pad = print_decimal(tmp);  // pone '0'
			pad = 15 - pad;
			while (pad--) {
				console_puts(" ");
			}
